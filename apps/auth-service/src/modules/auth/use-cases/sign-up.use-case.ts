...
import { generateRandomCodeNumber } from '@app/common/helpers/auth.helper';
import { makeSure, serverError } from '@app/common/helpers/server-error.helper';

import { EDriverStatus } from '@app/common/enums/driver.enum';
import {
  isValidEmail,
  isValidPhoneNumber
} from '@app/common/helpers/auth.helper';
// dto
...
export class SignUpUseCase {
  constructor(private driverProxyService: DriverProxyService) {}
  ...
  async signUpAccount(
    driver: SignUpDriverDto
  ): Promise<DriverDocumentWithCustomId> {
    await this.validateDriverDto(driver);
    try {
      const driverRegistered = await this.createDriver(driver);
  ...
  async findDriver(
    ...
    return await this.driverProxyService.findDriversByEmailOrPhoneNumber({
      email: driver.email,
  ...
  async createDriver(
    ...
    return await this.driverProxyService.createDriver(dataCreateDriver);
  }
}
