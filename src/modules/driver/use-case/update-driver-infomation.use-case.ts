  ...
  async validateUpdateDriverInformationDto(
    ...
    if (input?.fullName) {
      makeSure(
        ...
        EError.INVALID_FULL_NAME,
        EErrorDetail.INVALID_FULL_NAME
      );
    }
    // Kiểm tra email
    if (input?.email) {
    ...
    if (input?.activeAreaId) {
      makeSure(
        !isNaN(Number(input?.activeAreaId)),
        EError.INVALID_ACTIVE_AREA,
        EErrorDetail.INVALID_ACTIVE_AREA
      );
    ...
    if (input?.temporaryAddress) {
      makeSure(
        ...
        EErrorDetail.INVALID_TEMPORARY_ADDRESS
      );
